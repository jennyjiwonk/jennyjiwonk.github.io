...
// Change: the theme toggle now only cycles between "light" and "dark" (2 modes,
// sun/moon icons only) — the "system" setting and its half-sun-moon icon are
// removed. Original 3-state logic is commented out, not deleted. See
// docs/ARCHITECTURE.md#local-overrides-your-site-vs-this-repo. Run
// `bundle exec al-folio upgrade overrides diff assets/js/theme.js` after
// upgrading al_folio_core to check for upstream drift.
...
// Determine the expected state of the theme toggle, which is only ever "dark" or
// "light" now. Any stored value that isn't one of those two (including an old
// "system" value saved before this override) falls back to the OS preference,
// resolved once into a concrete choice.
let determineThemeSetting = () => {
  let themeSetting = localStorage.getItem("theme");
...
// determineThemeSetting() is always "dark" or "light" now, so the computed theme is
// just that setting directly.
let determineComputedTheme = () => {
...
let initTheme = () => {
  ...
  // Commented out: only relevant to the "system" setting, which no longer exists —
  // once a visitor's theme is explicitly "light" or "dark" it should stay put even
  // if their OS preference changes underneath them.
